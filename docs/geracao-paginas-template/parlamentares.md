...
| Suporte no `scripts/ingest.sh` (`pages-generate-parl`) | ✅ Implementado | `scripts/ingest.sh` |
...
```bash
bash scripts/ingest.sh --env container --mode pages-generate-parl
```
...
| Script modo | `pages-generate` / `pages-generate-ano` | `pages-generate-parl` |
